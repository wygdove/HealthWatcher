...
(function($) {
	$.fn.AjaxFileUpload = function(options) {
		...
		function onChange(e) {
			var $element = $(e.target), id = $element.attr('id'), $clone = $element
					.removeAttr('id').clone().attr('id', id).AjaxFileUpload(
							options), filename = $element.val().replace(
					/.*(\/|\\)/, ""), iframe = createIframe(), form = createForm(iframe);
			...
			if (data === false) {
				...
				for ( var variable in data) {
					$("<input />").attr("type", "hidden")
							.attr("name", variable).val(data[variable])
							.appendTo(e.target);
				}
			}
		}
		...
		function createIframe() {
			...
			$("body").append('<iframe src="javascript:false;" name="'+id+'" id="'
							+ id+'" style="display: none;"></iframe>');
